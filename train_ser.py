...
def main(args):
    ...
    print('*'*40)
    print(f"\n训练开始时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
...
def train(dataset, params, save_label='default'):
    ...
    print(f"训练开始: {datetime.datetime.now().strftime('%H:%M:%S')}")
    print(f"预计训练 {params['num_epochs']} 个epochs")
    print("-" * 50)
    ...
    for epoch in range(params['num_epochs']):
        ...
        print(f"\nEpoch {epoch+1}/{params['num_epochs']} - {datetime.datetime.now().strftime('%H:%M:%S')}")
        ...
        with tqdm(train_loader, desc=f"训练 Epoch {epoch+1}", ncols=100) as td:
        ...
        #Validation
        print(f"  验证阶段开始: {datetime.datetime.now().strftime('%H:%M:%S')}")
        ...
        remaining_epochs = params['num_epochs'] - (epoch + 1)
        ...
        print(f"  Epoch {epoch+1} 完成 (lr = {current_lr})")
        print(f"  Loss: {loss_format.format(train_loss)} -> {loss_format.format(val_loss)} | WA: {acc_format.format(val_wa)} <{acc_format.format(best_val_wa)}> | UA: {acc_format.format(val_ua)} <{acc_format.format(best_val_ua)}>")
        print(f"  时间: Epoch {epoch_time:.1f}s | 验证 {val_time:.1f}s | 总计 {elapsed_time/60:.1f}min | 预计剩余 {estimated_remaining/60:.1f}min")
        print(f"  最佳epoch: {best_epoch+1} (当前: {epoch+1})")
        
        # early stop
        if (epoch - best_epoch >= params['early_stop']) and (epoch > 5):
            print(f"\n  早停触发: 连续 {params['early_stop']} 个epoch无改善")
            break    
        #break
    ...
    print(f"最佳模型来自 Epoch {best_epoch+1}")
    print(f"开始测试阶段: {datetime.datetime.now().strftime('%H:%M:%S')}")
    ...
    with torch.no_grad():
        ...
        print(f"总耗时: {(total_training_time + test_time)/60:.1f} 分钟")
