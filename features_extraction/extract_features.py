...
def main(args):
    ...
    # Check if preprocessed pickle file already exists
    existing_pickle = os.path.join(dataset_dir, 'IEMOCAP_multi.pkl')
    if os.path.exists(existing_pickle):
        print(f'Loading existing preprocessed data from: {existing_pickle}')
        with open(existing_pickle, 'rb') as fin:
            features_data = pickle.load(fin)
        print(f'Loaded data with {len(features_data)} speakers')
        print(f'Speaker IDs: {list(features_data.keys())}')
    else:
        #Get file paths and label in database
        speaker_files = database.get_files()
        print(f'NUMBER OF FILES: {sum(len(files) for files in speaker_files.values())}')

        #Extract features
        features_data = extract_features(speaker_files, features, params)
        if features_data:
            print(f'Extracted features for {len(features_data)} speakers')
...
def parse_arguments(argv):
    ...
    # 默认路径 ../Datasets/IEMOCAP 表示从当前文件位置向上一级目录，然后进入 Datasets/IEMOCAP 文件夹
